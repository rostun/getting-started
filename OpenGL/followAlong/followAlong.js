...
function initGL(){
	...
	//will give you a 4 by 4 identity matrix using Sylvesters functions 
	var MVMatrix = Matrix.I(4);
	//Matrix.x multiplies a matrix by another matrix and then the matrix we're multipling by 
	//is a translation Matrix moving it -3 in the z direction
	//so everything in our square will be moved back into the scene by -3
	var trans = Matrix.Translation($V([0.0, 0.0, -6.0])).ensure4x4();
	MVMatrix = MVMatrix.x(trans); //v function takes an array so you need to give it an array
	//alert(JSON.stringify(MVMatrix.flatten())); //trans, MVMatrix, MVMatrix.flatten()
	//get unifrom location (vs attrdibute locatiosf rom before)
	var MVUniform = gl.getUniformLocation(program, "uMVMatrix");
	//set it location, normalized, data 
	//MVMatrix; sylvester stores it as something nicer, but you need it as a single array 
	//so float object will give it to you in that 
	gl.uniformMatrix4fv(MVUniform, false, new Float32Array(MVMatrix.flatten())); //constructor for Float32Array
